...
export default function AccordionItem(props) {
    ...
    return (
        <div className="accordion-item">
            <h2 className="accordion-header" onClick={clicked}>
                <button type="button" aria-expanded="true" className="accordion-button">
                    <span className="h-icon"><svg stroke="currentColor" fill="currentColor" strokeWidth="0"
            ...
            <div className={`accordion-collapse collapse ${status}`}>
                <div className="accordion-body">
                    <div className="row w-100 m-0">
                        {
                            props?.data.map((d, i) => {
                                return <div className="col-xl-2 col-lg-3 col-md-4 col-sm-6 mb-3" key={i}>
                                    <a href={d?.src ? d.src : `/donate/${d?.title}/0`}>
                                        <div><span>{d?.title}</span></div>
                                        <span className="icon"><svg className="donate-arrow" stroke="currentColor" fill="currentColor"
